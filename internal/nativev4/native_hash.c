...
const char *vipr_blake3_backend(void) { return VIPR_BLAKE3_BACKEND_NAME; }
...
vipr_status vipr_hash_file_tree(vipr_io_session *session, int use_patch_handle, uint64_t file_size,
    ...
    for (uint32_t index = 0; index < digest_count; ++index) {
        ...
        vipr_status status = vipr_read_at(session, handle, offset, buffer, length, error_buffer, error_buffer_size);
